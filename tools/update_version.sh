...
[ "$dev" = true ] && extra=".dev0"
case "$package" in
        json | matplotlib | plotly | plotly-express | ui | ui-experimental | utilities | packaging)
...
if [ "$dev" != true ]; then
    case "$package" in
        auth-keycloak | dashboard-object-viewer | matplotlib | plotly | plotly-express | table-example | ui | ui-experimental)
