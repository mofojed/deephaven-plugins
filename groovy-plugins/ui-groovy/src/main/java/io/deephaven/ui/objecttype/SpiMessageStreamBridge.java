 ...
 *
 * <p>If the deephaven-core SPI signatures shift between releases (e.g., the {@code onData}
 * parameter type, or {@code onClose} vs {@code close}), this adapter is the only place that needs
 * touching.
 */
public final class SpiMessageStreamBridge implements ClientConnection {
    ...
    /** The inbound-side bridge: wrap our {@link ElementMessageStream} so the SPI can deliver bytes to it. */
    public static MessageStream wrap(final ElementMessageStream stream) {
